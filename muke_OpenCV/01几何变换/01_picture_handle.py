...
img = cv2.imread("../../Picture/daimg.jpg", 1)
# 参数一:图片的路径
...
cv2.imwrite("../../Picture/daimg_copy.jpg", img)

# 图片压缩 - jpg   缺点: 1.有损压缩  2.无法设置透明度  优点: 压缩比高
img = cv2.imread("../../Picture/daimg.jpg", 1)
cv2.imwrite("../../Picture/daimg_copy.jpg", img, [cv2.IMWRITE_JPEG_QUALITY, 50])
# 第三个参数表示图片的质量, 有损压缩, 牺牲图片质量为代价, 范围是0-100, 数值越小, 图片大小越小, 失真度越高

# 图片压缩 - png   缺点: 压缩比底   优点: 1.无损压缩  2.可设置透明度
img = cv2.imread("../../Picture/daimg.jpg", 1)
cv2.imwrite("../../Picture/daimg_copy.png", img, [cv2.IMWRITE_PNG_COMPRESSION, 9])
